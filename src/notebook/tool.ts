...
export const notebookToolInputSchema = z.object({
  ...
  instanceId: z.string().optional().describe("Runbook instance ID (ordered instance-aware execution for notebook_run_cell)"),
  templateId: z.string().optional().describe("Runbook template ID — the source notebook's id (notebook_fitness)"),
  templateVersion: z.number().int().optional().describe("Runbook template version (notebook_fitness)"),
