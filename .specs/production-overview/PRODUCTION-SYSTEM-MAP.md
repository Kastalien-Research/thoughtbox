...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MCP server | Node 22, Express, MCP streamable HTTP | `src/index.ts`, `node dist/index.js` | Root `Dockerfile`, `infra/gcp/mcp-service.tf` (Terraform) | Image: Cloud Build trigger on push to main (`google_cloudbuild_trigger.mcp_deploy`); service config: Terraform `infra/gcp/` | `PORT`, `BASE_URL`, `THOUGHTBOX_STORAGE`, `NODE_ENV`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_JWT_SECRET`, `SUPABASE_SERVICE_ROLE_KEY`, `OAUTH_JWT_SECRET`, `LANGSMITH_API_KEY`, `TB_BRANCH_SIGNING_SECRET`, `DISABLE_THOUGHT_LOGGING` | Supabase in deployed mode; filesystem default for local mode | `/health`, `/info`, unauthenticated `/mcp` | `src/index.ts`, `Dockerfile`, `infra/gcp/mcp-service.tf`, live probes | `confirmed-current` |
...
| Direct Cloud Run MCP URL | Google Cloud Run | `https://thoughtbox-mcp-272720136470.us-central1.run.app` | `infra/gcp/mcp-service.tf` (service + domain mapping for `mcp.kastalienresearch.ai`) | Terraform `infra/gcp/` | Same as MCP server | Same as MCP server | `/health`, `/info`, `/mcp` auth error | live probe, web quickstart docs | `confirmed-current` |
...
- `infra/gcp/mcp-service.tf` (Terraform-managed Cloud Run service) sets `THOUGHTBOX_STORAGE=supabase`.
