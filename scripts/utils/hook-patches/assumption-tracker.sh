...
if [[ "$is_candidate" == "true" ]]; then
    ...
    # Truncate claim to 200 chars, escape for JSON
    safe_claim=$(echo "$claim" | head -c 200 | sed 's/"/\\"/g' | tr -d '\n')
    echo "{\"timestamp\":\"$timestamp\",\"category\":\"$category\",\"claim\":\"$safe_claim\",\"source\":\"auto-detected\",\"status\":\"candidate\"}" >> "$candidates_file"
fi
