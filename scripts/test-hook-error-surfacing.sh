...
echo ""
echo "Test 4: ulysses_state_writer handles corrupted current-task.json"
state_dir="$PROJECT_DIR/.claude/state/task-workflow"
mkdir -p "$state_dir"
# Create a corrupted current-task.json
echo "CORRUPTED" > "$state_dir/current-task.json"
...
rm -f "$state_dir/current-task.json" "$state_dir/current-task.json.lock"
