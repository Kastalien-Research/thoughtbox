...
interface CanonicalIR {
  ...
  branch: string;
  agent: string;
  refs: string[];          // Lightweight direct references
  confidence?: number;
  ...
  attrs: Record<string, any>; // Type-specific fields
}
```
...
6. **Response:** The backend returns compact `TBX-C1` results (e.g., columnar query data), which the host decode or passes to the LLM.
